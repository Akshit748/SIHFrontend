...
// ─── Timeline events (demo — clearly labelled when displayed) ─────────────────
// Timestamps relative to ~45 s before now, matching elapsedMs in mission state.

const T0 = Date.now() - 45_000;

const BASE_TIMELINE: TimelineEvent[] = [
  { id: 'e1',  timestampMs: T0 + 0,      level: 'info',    message: 'System initialized — ROS 2 Humble' },
  { id: 'e2',  timestampMs: T0 +  1_800, level: 'info',    message: 'RTAB-Map started — occupancy grid building' },
  { id: 'e3',  timestampMs: T0 +  3_200, level: 'success', message: 'Localization ready — map→odom TF active' },
  { id: 'e4',  timestampMs: T0 +  5_100, level: 'info',    message: 'Nav2 action server ready' },
  { id: 'e5',  timestampMs: T0 +  8_400, level: 'success', message: 'Goal received: (−6.21, 0.63) map frame' },
  { id: 'e6',  timestampMs: T0 +  8_900, level: 'info',    message: 'Path generated — GridBased planner' },
  { id: 'e7',  timestampMs: T0 +  9_200, level: 'info',    message: 'Navigating…' },
  { id: 'e8',  timestampMs: T0 + 22_000, level: 'warn',    message: 'Nav2 recovery behaviour triggered (1/3)' },
  { id: 'e9',  timestampMs: T0 + 33_500, level: 'warn',    message: 'Nav2 recovery behaviour triggered (2/3)' },
  { id: 'e10', timestampMs: T0 + 34_100, level: 'info',    message: 'Resuming — replanned path OK' },
];
...
export const INITIAL_APP_STATE: AppState = {
  ...
  mission: {
    ...
    recoveries: 2,  // 3 total in the recorded run; 2 have fired so far in demo
    elapsedMs: 45_000,
  },
